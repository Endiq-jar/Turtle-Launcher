...
static bool init_exit_hook() {
    ...
    if(bhook_status == BYTEHOOK_STATUS_CODE_OK) {
        ...
        // These two hooks are the Amethyst SDL fix.  SDL is loaded by the
        // embedded game JVM, but its Android frontend belongs to ART.  Install
        // them before the game loads LWJGL so the first SDL init can notify the
        // launcher and a guest JNI_OnLoad cannot overwrite ART's SDL state.
...
Java_com_movtery_turtlelauncher_bridge_TurtleBridge_initializeGameExitHook(JNIEnv *env, jclass clazz) {
