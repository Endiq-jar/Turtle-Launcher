 ...
 * process dies and there is nothing to restore.
 *
 * Deliberately scoped to the game session only (installed right before launchJVM, removed
 * right after it returns) so ordinary launcher-side logging never gets mirrored into the
 * game log.
 */
