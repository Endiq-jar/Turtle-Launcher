...
Java_com_endiq_turtlelauncher_presentation_util_dns_DnsHookNative_installResolvConfRedirect(
 ...
 */
JNIEXPORT void JNICALL
Java_com_endiq_turtlelauncher_presentation_util_dns_DnsHookNative_installHostsRedirect(
        JNIEnv* env, jclass clazz, jstring jpath) {
    const char* path = (*env)->GetStringUTFChars(env, jpath, NULL);
