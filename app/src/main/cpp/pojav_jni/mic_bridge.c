 ...
 * com.endiq.turtlelauncher.audio.TurtleMicNative 클래스의 native 메서드를
...
Java_com_endiq_turtlelauncher_audio_TurtleMicNative_nativeOpen(
...
Java_com_endiq_turtlelauncher_audio_TurtleMicNative_nativeRead(
...
Java_com_endiq_turtlelauncher_audio_TurtleMicNative_nativeClose(JNIEnv *env, jclass clazz) {
    (void) env; (void) clazz;
    pthread_mutex_lock(&g_mic_lock);
    if (g_mic_stream != NULL) {
        AAudioStream_requestStop(g_mic_stream);
...
JNIEXPORT jboolean JNICALL
Java_com_endiq_turtlelauncher_audio_TurtleMicNative_nativeIsAvailable(JNIEnv *env, jclass clazz) {
    (void) env; (void) clazz;
