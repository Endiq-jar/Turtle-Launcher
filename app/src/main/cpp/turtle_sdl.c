...
#define LOG(...) __android_log_print(ANDROID_LOG_INFO, "TURTLE_SDL", __VA_ARGS__)
...
enum {
    TURTLE_SDL_KEY_DOWN = 0x300, TURTLE_SDL_KEY_UP = 0x301, TURTLE_SDL_TEXT_INPUT = 0x303,
    TURTLE_SDL_MOUSE_MOTION = 0x400, TURTLE_SDL_BUTTON_DOWN = 0x401, TURTLE_SDL_BUTTON_UP = 0x402,
    TURTLE_SDL_MOUSE_WHEEL = 0x403,
};

static struct {
    bool     (*PushEvent)(TurtleSDLEvent *);
...
static bool turtle_sdlBind(void) {
    if (sdl.PushEvent) return true;
    void *h = dlopen("libSDL3.so", RTLD_NOLOAD | RTLD_LAZY);
    if (!h) return false;
#define TURTLE_SDL_LOAD(name) sdl.name = (__typeof__(sdl.name))dlsym(h, "SDL_" #name)
    TURTLE_SDL_LOAD(GetKeyboardFocus);
    TURTLE_SDL_LOAD(GetWindowID);
    TURTLE_SDL_LOAD(GetWindowSize);
    TURTLE_SDL_LOAD(GetWindowRelativeMouseMode);
    TURTLE_SDL_LOAD(WarpMouseInWindow);
    TURTLE_SDL_LOAD(GetKeyFromScancode);
    TURTLE_SDL_LOAD(SetModState);
    TURTLE_SDL_LOAD(SetMainReady);
    TURTLE_SDL_LOAD(PushEvent);   // 마지막 — 이게 서 있으면 나머지도 채워진 것이다
#undef TURTLE_SDL_LOAD
    return sdl.PushEvent != NULL;
}

/// 게임의 SDL 창. 없으면 NULL — 아직 부팅 중이거나 26.3 이 아니다.
static void *turtle_sdlWindow(void) {
    static void *window;
    if (!turtle_sdlBind()) return NULL;
...
static uint16_t turtle_sdlMods;
...
JNIEXPORT jboolean JNICALL J(nativeSdlSendKey)(JNIEnv *env, jobject thiz, jint scancode, jint action) {
    ...
    if (scancode >= 224 && scancode <= 231) {
        ...
        turtle_sdlMods = down ? (turtle_sdlMods | bit) : (turtle_sdlMods & ~bit);
        sdl.SetModState(turtle_sdlMods);
    }

    TurtleSDLEvent e = {0};
    e.key.type = down ? TURTLE_SDL_KEY_DOWN : TURTLE_SDL_KEY_UP;
    e.key.windowID = sdl.GetWindowID(w);
    e.key.scancode = scancode;
    e.key.key = sdl.GetKeyFromScancode(scancode, turtle_sdlMods, true);
    e.key.mod = turtle_sdlMods;
    e.key.down = down;
...
JNIEXPORT jboolean JNICALL J(nativeSdlSendChar)(JNIEnv *env, jobject thiz, jint codepoint) {
    ...
    TurtleSDLEvent e = {0};
    e.text.type = TURTLE_SDL_TEXT_INPUT;
...
JNIEXPORT jboolean JNICALL J(nativeSdlSendMouseButton)(JNIEnv *env, jobject thiz, jint glfwButton, jint action) {
    ...
    TurtleSDLEvent e = {0};
    e.button.type = action ? TURTLE_SDL_BUTTON_DOWN : TURTLE_SDL_BUTTON_UP;
...
JNIEXPORT jboolean JNICALL J(nativeSdlSendCursorPos)(JNIEnv *env, jobject thiz, jint mode, jfloat x, jfloat y) {
    ...
    if (mode == 1) {
        TurtleSDLEvent e = {0};
        e.motion.type = TURTLE_SDL_MOUSE_MOTION;
    ...
    int ww = 0, wh = 0;
    if (turtle_fbWidth > 0 && turtle_fbHeight > 0 && sdl.GetWindowSize(w, &ww, &wh)) {
        sdl.WarpMouseInWindow(w, x * ww / turtle_fbWidth, y * wh / turtle_fbHeight);
    } else {
...
JNIEXPORT jboolean JNICALL J(nativeSdlSendScroll)(JNIEnv *env, jobject thiz, jfloat dx, jfloat dy) {
    ...
    TurtleSDLEvent e = {0};
    e.wheel.type = TURTLE_SDL_MOUSE_WHEEL;
...
JNIEXPORT jboolean JNICALL J(nativeSdlSetMainReady)(JNIEnv *env, jobject thiz) {
    (void)env; (void)thiz;
    if (!turtle_sdlBind() || !sdl.SetMainReady) return JNI_FALSE;
    sdl.SetMainReady();
