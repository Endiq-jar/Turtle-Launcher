package com.endiq.turtlelauncher.presentation.util.forge
...
//    BootstrapLauncher 의 자동 모듈 등록 단계에서 split package 충돌이 줄줄이 발생.
        ...
        if (profile.requiresProcessors) {
            try {
                ...
                processorMain = "com.endiq.turtlelauncher.forge.ProcessorLauncher"
    ...
    private fun copyProcessorLauncherJar(context : Context, librariesDir: File): File {
        val dest = File(librariesDir, "com/endiq/turtlelauncher/processor-launcher/1.0/processor-launcher-1.0.jar")
        dest.parentFile?.mkdirs()
        if (!dest.exists() || dest.length() == 0L) {
