...
object CustomVulkanDriverManager {
    ...
    fun importDriverZip(context: Context, zipUri: Uri): DriverInfo? {
        ...
        return try {
            ...
            if (!metaFile.exists()) {
                Log.e("TURTLE_LAUNCHER", "커스텀 드라이버 zip 안에 meta.json 이 없음")
                targetDir.deleteRecursively()
                return null
            }
            ...
            if (libraryName.isNullOrBlank() || !File(targetDir, libraryName).exists()) {
                Log.e("TURTLE_LAUNCHER", "meta.json 의 libraryName 이 없거나 해당 .so 파일이 없음")
                targetDir.deleteRecursively()
                return null
            }
            ...
            Log.e("TURTLE_LAUNCHER", "커스텀 드라이버 가져오기 실패: ${e.message}", e)
            targetDir.deleteRecursively()
            null
        }
    }
