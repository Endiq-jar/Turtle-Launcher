package com.endiq.turtlelauncher.data.jvm
...
data class JvmSettings(
    ...
    // ── High-performance game profile ─────────────────────────────
    // These values are written to Minecraft's options.txt before launch. They are deliberately
    // explicit so users can tune the profile instead of relying on hidden magic constants.
    val maxFps: Int = 1000,
    ...
    fun toJvmArgArray(
        ...
        args += listOf(
            ...
            "-Dorg.lwjgl.util.NoChecks=$fastMath",
            "-Dorg.lwjgl.util.Debug=$rendererDebugLogging",
            "-Dorg.lwjgl.util.DebugLoader=$rendererDebugLogging",
            "-Dfml.earlyprogresswindow=false",
            "-Djava.awt.headless=false",
            "-Dsun.java2d.opengl=false",
            "-Djava.util.concurrent.ForkJoinPool.common.parallelism=${Runtime.getRuntime().availableProcessors().coerceAtLeast(2)}",
        ...
        if (fastMath) {
            args += listOf(
                "-Dfastutil.arrayGrowthFactor=1.25",
                "-Dorg.lwjgl.util.NoChecks=true",
                "-Dnet.minecraft.client.render.chunk.compileThreadCount=${if (threadedChunkBuild) Runtime.getRuntime().availableProcessors().coerceIn(2, 8) else 1}"
            )
        }
        args += listOf(
            "-Dturtle.framePacing=$framePacing",
            "-Dturtle.frameSkipping=$frameSkipping",
            "-Dturtle.adaptiveFrameTiming=$adaptiveFrameTiming",
            "-Dturtle.lowLatencyRendering=$lowLatencyRendering",
            "-Dturtle.threadedChunkBuild=$threadedChunkBuild",
        )
        if (shaderCache) {
            args += listOf(
                "-Dsodium.console=false",
                "-Diris.shaderPackDevelopment=false",
                "-Dorg.lwjgl.opengl.shadercache=true"
            )
        }
...
object JvmSettingsManager {
    ...
    fun load(context: Context): JvmSettings {
        ...
        return try {
            ...
            // New extreme-FPS fields must retain their Kotlin defaults when loading an older
            // Gson file; Gson otherwise fills omitted booleans/ints with false/zero.
            if (obj?.has("maxFps") != true) settings = settings.copy(maxFps = 1000)
            if (obj?.has("enableVsync") != true) settings = settings.copy(enableVsync = false)
            if (obj?.has("adaptiveFrameTiming") != true) settings = settings.copy(adaptiveFrameTiming = true)
            if (obj?.has("lowLatencyRendering") != true) settings = settings.copy(lowLatencyRendering = true)
            if (obj?.has("performanceProfileLocked") != true) settings = settings.copy(performanceProfileLocked = true)
            if (obj?.has("entityDistanceScaling") != true) settings = settings.copy(entityDistanceScaling = 0.5f)
            if (obj?.has("simulationDistance") != true) settings = settings.copy(simulationDistance = 4)
            if (obj?.has("particles") != true) settings = settings.copy(particles = 2)
            if (obj?.has("shaderCache") != true) settings = settings.copy(shaderCache = true)
            if (obj?.has("fastMath") != true) settings = settings.copy(fastMath = true)
            if (obj?.has("threadedChunkBuild") != true) settings = settings.copy(threadedChunkBuild = true)
            ...
            settings = settings.copy(
                ...
                maxFps = settings.maxFps.coerceIn(30, 1000),
                renderDistance = settings.renderDistance.coerceIn(2, 32),
                simulationDistance = settings.simulationDistance.coerceIn(2, 32),
                mipmapLevels = settings.mipmapLevels.coerceIn(0, 4),
                entityDistanceScaling = settings.entityDistanceScaling.coerceIn(0.25f, 1f),
                particles = settings.particles.coerceIn(0, 2),
                biomeBlendRadius = settings.biomeBlendRadius.coerceIn(0, 7),
            )

            // cacheDirPath가 비어있으면 채워주기
            settings = if (settings.cacheDirPath.isEmpty()) {
