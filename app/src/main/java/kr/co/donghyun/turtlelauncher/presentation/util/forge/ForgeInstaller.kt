package kr.co.donghyun.turtlelauncher.presentation.util.forge
...
class ForgeInstaller(
    ...
    fun install(context : Context, mcVersion: String, forgeVersion: String, isNeoForge: Boolean = false): ForgeInstallResult {
        ...
        val parsed = try {
            ...
            Log.e("TURTLE_LAUNCHER", "installer 파싱 실패", e)
...
//    BootstrapLauncher 의 자동 모듈 등록 단계에서 split package 충돌이 줄줄이 발생.
        ...
        if (profile.requiresProcessors) {
            try {
                ...
                processorMain = "kr.co.donghyun.turtlelauncher.forge.ProcessorLauncher"

                Log.i("TURTLE_LAUNCHER", "✅ Forge processors 메타 준비 완료 (빌더 cp=${processorCp.size}개, 게임 진입점=$gameMainClass)")
            } catch (e: Exception) {
                Log.e("TURTLE_LAUNCHER", "Processor 메타 준비 실패", e)
                return ForgeInstallResult(success = false,
    ...
    private fun extractBundledMaven(installerJar: File, librariesDir: File) {
        try {
            ...
            Log.w("TURTLE_LAUNCHER", "bundled maven 추출 실패: ${e.message}")
        }
    }
    ...
    private fun preDownloadMojmaps(
        ...
        if (!mojmapsRaw.startsWith("[") || !mojmapsRaw.endsWith("]")) {
            Log.w("TURTLE_LAUNCHER", "🗺 MOJMAPS 좌표 형식 이상: $mojmapsRaw")
            return null
        }
        ...
        val versionEntry = try {
            ...
            Log.w("TURTLE_LAUNCHER", "🗺 version manifest 에서 $mcVersion 못 찾음"); return null
        }
        ...
        val versionJson = try {
            ...
            Log.e("TURTLE_LAUNCHER", "🗺 version.json 실패: ${e.message}"); return null
        } ?: return null
        ...
        if (cm == null) {
            Log.w("TURTLE_LAUNCHER", "🗺 client_mappings 없음 (구버전?)")
            return null
        }
        ...
        if (target.exists() && target.length() > 0) {
            ...
            if (mojangSha?.equals(current, ignoreCase = true) == true) {
                Log.i("TURTLE_LAUNCHER", "🗺 ✅ mojmaps(=Mojang 현재 SHA) OK — pre-download skip")
                return current.lowercase()
            }
            // Mojang 현재값과 다르면(=stale 한 install_profile SHA 로 받아둔 구버전 등) 재다운로드.
            Log.w("TURTLE_LAUNCHER", "🗺 디스크 mojmaps 가 Mojang 현재 SHA 와 불일치 → 재다운로드")
        }

        Log.i("TURTLE_LAUNCHER", "🗺 mojmaps pre-download: $url → ${target.absolutePath}")
        target.parentFile?.mkdirs()
        return try {
            client.newCall(Request.Builder().url(url).build()).execute().use { resp ->
                if (!resp.isSuccessful) {
                    Log.e("TURTLE_LAUNCHER", "🗺 HTTP ${resp.code}"); return null
                }
                resp.body?.byteStream()?.use { input ->
            ...
            when {
                mojangSha?.equals(actual, ignoreCase = true) == true -> {
                    Log.i("TURTLE_LAUNCHER", "🗺 ✅ Mojang version.json SHA 와 일치")
                }
                installProfileExpectedSha?.equals(actual, ignoreCase = true) == true -> {
                    Log.i("TURTLE_LAUNCHER", "🗺 ✅ install_profile SHA 일치")
                }
                else -> {
            ...
            Log.e("TURTLE_LAUNCHER", "🗺 OkHttp 다운로드 예외: ${e.message}", e)
            null
        }
    }
    ...
    private fun downloadLibrary(lib: LibSpec, librariesDir: File): File? {
        ...
        Log.w("TURTLE_LAUNCHER", "라이브러리 다운로드 실패: ${lib.name}")
        return null
    }
    ...
    private fun downloadForgeUniversal(
        ...
        Log.w("TURTLE_LAUNCHER", "Forge universal jar 다운로드 실패: $fullVersion")
        return null
    }
    ...
    private fun copyProcessorLauncherJar(context : Context, librariesDir: File): File {
        val dest = File(librariesDir, "kr/co/donghyun/turtlelauncher/processor-launcher/1.0/processor-launcher-1.0.jar")
        dest.parentFile?.mkdirs()
        if (!dest.exists() || dest.length() == 0L) {
