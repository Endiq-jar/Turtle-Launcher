...
import kr.co.donghyun.turtlelauncher.data.mojang.MCPrepareResult
import kr.co.donghyun.turtlelauncher.data.mojang.VersionEntry
import kr.co.donghyun.turtlelauncher.data.mojang.VersionManifest
...
class MinecraftDownloader(
    ...
    fun prepare(): MCPrepareResult {
        ...
        Log.d("TURTLE_LAUNCHER", "✅ MC ${manifest.id} 준비 완료 → ${instanceDir.absolutePath}")
    ...
    private fun downloadFile(
        ...
        if (destFile.exists() && destFile.length() > 0) {
            ...
            Log.w("TURTLE_LAUNCHER", "필수 GLFW 콜백 jar 캐시가 불완전하여 다시 다운로드합니다: $destFile")
            destFile.delete()
        }
    ...
    private fun containsGlfwCallbackClasses(file: File): Boolean {
        return try {
            ...
            Log.w("TURTLE_LAUNCHER", "필수 GLFW 콜백 jar 검증 실패: $file", error)
            false
        }
    }
