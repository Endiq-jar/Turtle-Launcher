package kr.co.donghyun.turtlelauncher.presentation.ui.theme
...
//    아예 없었다 — 화면들이 TurtlePrimary 같은 값을 직접 갖다 쓰고 있어서 드러나지 않았을 뿐이다.
//
//    런처는 게임 화면(항상 어둡다) 옆에 붙어 있는 UI라 밝은 테마가 의미가 없고,
//    두 플랫폼의 화면이 같아 보여야 하므로 iOS 와 같이 다크 하나로 고정한다.
private val TurtleDarkScheme = darkColorScheme(
    primary = TurtlePrimary,
    onPrimary = BgDark,
    primaryContainer = TurtleDark,
    onPrimaryContainer = TextPrimary,

    secondary = TurtleLight,
    onSecondary = BgDark,

    tertiary = TurtleAccent,
    onTertiary = BgDark,
