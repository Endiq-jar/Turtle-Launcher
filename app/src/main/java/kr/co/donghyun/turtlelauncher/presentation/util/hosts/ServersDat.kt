...
object ServersDat {
    ...
    fun read(file: File): List<ServerEntry> {
        ...
        return try {
            ...
            Log.w("TURTLE_LAUNCHER", "servers.dat 읽기 실패: ${e.message}")
            emptyList()
        }
    }
    ...
    fun upsert(file: File, entry: ServerEntry) {
        ...
        write(file, current)
        Log.d("TURTLE_LAUNCHER", "servers.dat 갱신: ${entry.name} (${entry.ip}) → 총 ${current.size}개")
    }
    ...
    fun removeByIp(file: File, ip: String) {
        ...
        if (removed) {
            write(file, current)
            Log.d("TURTLE_LAUNCHER", "servers.dat 에서 제거: $ip")
        }
    }
}