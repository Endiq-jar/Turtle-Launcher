...
import kr.co.donghyun.turtlelauncher.data.mods.ModrinthProject
import kr.co.donghyun.turtlelauncher.data.mods.ModrinthSearchHit
import kr.co.donghyun.turtlelauncher.data.mods.ModrinthSearchResponse
...
class ModrinthAPI {
    ...
    private val userAgent = "donghyun/TurtleLauncher/1.0 (kr.co.donghyun.turtlelauncher)"
        ...
        return try {
            client.newCall(buildRequest(url)).execute().use { resp ->
                ...
                if (!resp.isSuccessful) {
                    Log.w("TURTLE_LAUNCHER", "Modrinth 검색 실패 ${resp.code}: $json")
                    return emptyList()
                }
                gson.fromJson(json, ModrinthSearchResponse::class.java)?.hits ?: emptyList()
            }
        } catch (e: Exception) {
            Log.e("TURTLE_LAUNCHER", "Modrinth 검색 예외: ${e.message}")
            emptyList()
        }
    }
    ...
    fun getProject(idOrSlug: String): ModrinthProject? {
        ...
        return try {
            ...
            Log.e("TURTLE_LAUNCHER", "Modrinth 프로젝트 조회 예외: ${e.message}")
            null
        }
    }
    ...
    fun getVersions(
        ...
        return try {
            ...
            Log.e("TURTLE_LAUNCHER", "Modrinth 버전 조회 예외: ${e.message}")
            emptyList()
        }
    }
    ...
    fun getVersion(versionId: String): ModrinthVersion? {
        ...
        return try {
            ...
            Log.e("TURTLE_LAUNCHER", "Modrinth 단일 버전 조회 예외: ${e.message}")
            null
        }
    }
