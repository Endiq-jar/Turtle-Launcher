package kr.co.donghyun.turtlelauncher.presentation.util
...
class PrepareNatives {
    companion object {
        fun prepareNatives(applicationContext : Context, applicationInfo : ApplicationInfo) {
            ...
            Log.d("TURTLE_LAUNCHER", "natives mkdirs: $mkdirResult, exists=${nativesDir.exists()}")


            val apkLibDir = File(applicationInfo.nativeLibraryDir)
            Log.d("TURTLE_LAUNCHER", "apkLibDir: ${apkLibDir.absolutePath}, exists=${apkLibDir.exists()}")
            val files = apkLibDir.listFiles()
            Log.d("TURTLE_LAUNCHER", "apkLibDir files: ${files?.size}")
