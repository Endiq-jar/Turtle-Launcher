...
import kr.co.donghyun.turtlelauncher.presentation.instancesettings.InstanceSettingsViewModel
import kr.co.donghyun.turtlelauncher.presentation.ui.screen.InstanceSettingsScreen
...
class InstanceSettingsActivity : BaseActivity() {
    ...
    private fun launchMapPicker() {
        try {
            ...
            Log.e("TURTLE_LAUNCHER", "맵 피커 실행 실패: ${e.message}", e)
    ...
    private fun launchModPicker() {
        try {
            ...
            Log.e("TURTLE_LAUNCHER", "모드 피커 실행 실패: ${e.message}", e)
            Toast.makeText(this, getString(R.string.cannot_open_file_picker), Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun launchModpackExporter() {
        ...
        try {
            ...
            Log.e("TURTLE_LAUNCHER", "모드팩 내보내기 피커 실행 실패: ${e.message}", e)
    ...
    private fun launchModpackImporter() {
        try {
            ...
            Log.e("TURTLE_LAUNCHER", "모드팩 가져오기 피커 실행 실패: ${e.message}", e)
            Toast.makeText(this, getString(R.string.cannot_open_file_picker), Toast.LENGTH_SHORT).show()
        }
    }
